...
export const initializeWebSocket = (server: http.Server) => {
  ...
  io.on("connection", (socket: AuthenticatedSocket) => {
    ...
    socket.on(
      ...
      async ({
        ...
        try {
          ...
          // Validate input
          if (!message.trim())
            throw new CustomError(400, "Message cannot be empty");

          // Validate ticket access
          const hasAccess = await validateTicketAccess(
            socket.userId!,
            ticketId
          );
    ...
    socket.on("typing", ({ ticketId }: { ticketId: string }) => {
      socket
        .to(ticketId)
        .emit("userTyping", { userId: socket.userId, ticketId });
    });
    ...
    socket.on("stopTyping", ({ ticketId }: { ticketId: string }) => {
      socket
        .to(ticketId)
        .emit("userStoppedTyping", { userId: socket.userId, ticketId });
    });
        ...
        try {
          ...
          if (
            !message.readBy.some((id) =>
              id.equals(new Types.ObjectId(socket.userId!))
            )
          ) {
          ...
          io.to(ticketId).emit("messageRead", {
            ticketId,
            messageId,
    ...
    socket.on(
      ...
      async ({
        ...
        try {
          ...
          const hasUpdatePermission = await checkPermission(
            socket.userId!,
            "tickets",
            2
          );
          ...
          if (!hasUpdatePermission && !isAdmin)
            throw new CustomError(403, "No permission to update ticket");
...
async function validateTicketAccess(
  ...
  return (
    isAdmin || hasTicketAccess || Boolean(isCustomer) || Boolean(isAssigned)
  );
}
